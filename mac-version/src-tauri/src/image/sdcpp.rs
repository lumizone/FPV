...
/// master — matches the win fork's `sd-cli.exe` minus the extension).
const SD_EXE: &str = "sd-cli";

/// Resolve the bundled sd-cli. Absolute paths only, never PATH (hard
/// rule 93): the sandboxed .app inherits a minimal PATH and must not
/// exec anything outside its own bundle anyway.
///   1. next to the running binary (`Contents/MacOS/sd-cli` — staged +
///      signed by scripts/package.sh)
///   2. the dev tree (`src-tauri/binaries/sd-cli`, for `tauri dev`)
...
pub async fn generate(req: crate::image::ImageRequest) -> AppResult<crate::image::ImageResult> {
    ...
    let mut child = tokio::process::Command::new(&cli)
        ...
        .spawn()
        .map_err(|e| AppError::Other(format!("sd-cli spawn failed: {e}")))?;
...
mod tests {
    ...
    fn sdxl_args_load_single_file_with_sampler_and_dims() {
        ...
        let s = args.join(" ");
        assert!(s.contains("--model /w/animagine-xl-4.0-opt.safetensors"));
        assert!(s.contains("--vae /w/sdxl_vae_fp16_fix.safetensors"));
    ...
    fn flux_args_are_byte_identical_to_before() {
        ...
        // reorder/insertion on the FLUX path fails loudly.
        let args = build_sd_args(
            LocalModel::Klein4B,
            Path::new("/w"),
            "a cat",
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/w/flux-2-klein-4b-Q4_0.gguf".to_string(),
                "--llm".to_string(),
                "/w/Qwen3-4B-Q4_K_M.gguf".to_string(),
                "--vae".to_string(),
                "/w/flux2_ae.safetensors".to_string(),
                "-p".to_string(),
                "a cat".to_string(),
                "--cfg-scale".to_string(),
                "1".to_string(),
