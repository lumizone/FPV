...
export default function App() {
  ...
  // Windows GPU-runtime provisioning toast. Fires for BOTH Ollama's own
  // GPU runner and the local image backend's sd.cpp GPU build — they
  // share one "gpu-setup" channel and phase vocabulary
  // (downloading/ready/error) since a generic "setting up GPU
  // acceleration" toast doesn't need to say which subsystem triggered it.
  // Never fires on macOS (nothing backend-side ever emits it there), so
  // this listener is harmlessly idle rather than platform-gated.
  useEffect(() => {
    ...
    import("@tauri-apps/api/event")
      .then(({ listen }) =>
        listen<string>("gpu-setup", (e) => {
          if (e.payload === "downloading") {
            toast.loading(t("app.gpu_setup.downloading", "Setting up GPU acceleration…"), { id: "gpu-setup" });
          } else if (e.payload === "ready") {
            toast.success(t("app.gpu_setup.ready", "GPU acceleration ready"), { id: "gpu-setup" });
          } else if (e.payload === "error") {
            toast.error(t("app.gpu_setup.error", "GPU acceleration setup failed — continuing on CPU"), { id: "gpu-setup" });
          }
        })
      )
