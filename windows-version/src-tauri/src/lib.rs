...
pub fn run() {
    ...
    // Windows only: create the process-wide Job Object now, so the
    // bundled Ollama sidecar (spawned later, on demand) can be assigned
    // to it and a creation failure is logged at boot rather than
    // silently deferred to the first spawn. See sidecars/job_object.rs.
    #[cfg(windows)]
    crate::sidecars::job_object::init();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // executables inside FPV.app bundles (macOS-only: on Windows the
            // Job Object reaps everything on abnormal app exit).
            #[cfg(not(windows))]
            ...
            // Windows only: fetch the vendor-matched GPU build for local
            // image generation, gated on the local provider actually being
            // selected and weights already cached. Without this, a user
            // whose weights are already downloaded has no reachable path
            // that ever provisions the GPU runtime — see
            // `sidecars::gpu_runtime::spawn_boot_provision`.
            #[cfg(windows)]
            {
                let db = app.state::<AppState>().inner().db.clone();
                crate::sidecars::gpu_runtime::spawn_boot_provision(handle.clone(), db);
            }

            // Windows only: fetch the vendor-matched Ollama GPU runner
            // (Blackwell CUDA v13 / AMD ROCm) — the bundled runtime only
            // carries CPU + a single pruned CUDA line to fit the NSIS size
            // limit. A DIFFERENT provisioning concern from the one above
            // (Ollama's own runtime vs. the image backend's), sharing only
            // GPU vendor detection. See `sidecars::ollama_gpu_runtime`.
            #[cfg(windows)]
            {
                let db = app.state::<AppState>().inner().db.clone();
                crate::sidecars::ollama_gpu_runtime::spawn(handle.clone(), db);
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::hardware::gpu_runtime_status,
            commands::hardware::gpu_runtime_retry_ollama,
            commands::hardware::gpu_runtime_retry_image,
