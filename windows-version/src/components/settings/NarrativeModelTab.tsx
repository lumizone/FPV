...
export function NarrativeModelTab() {
  ...
  // Windows-only Ollama GPU-runner status (Blackwell CUDA v13 / AMD ROCm —
  // the bundled runtime only carries CPU + a single pruned CUDA line).
  // `gpuRuntimeStatus()` itself is safe to call on any platform: it
  // returns `platform_supported: false` on macOS rather than erroring.
  const refreshGpu = () => {
  ...
  const handleGpuRetry = async () => {
    ...
    setGpuRetrying(true);
    try {
      await gpuRuntimeRetryOllama();
      refreshGpu();
    } catch (e: any) {
      toast.error(
        (e as { message?: string })?.message ??
          t("settings.models.gpu_retry_failed", "GPU setup retry failed — continuing on CPU.")
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Windows-only: Ollama GPU acceleration status. Metal on macOS
          needs no provisioning step, so this card only makes sense on
          Windows — the bundled Ollama runtime only carries CPU + a single
          CUDA line, so newer NVIDIA (Blackwell) and AMD GPUs need a
          one-time runner download to actually accelerate. */}
      {isWindows && gpuStatus?.platform_supported && (
        <div className="surface p-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Cpu className={`w-4 h-4 shrink-0 ${gpuStatus.ollama_accelerated ? "text-[var(--color-system-green)]" : "text-[var(--color-label-tertiary)]"}`} />
            <div className="min-w-0">
              <div className="text-[13px] font-medium text-[var(--color-label-primary)]">
                {t("settings.models.gpu_title", "GPU acceleration")}
              </div>
              <div className="text-[11px] text-[var(--color-label-tertiary)]">
                {gpuStatus.vendor === "other"
                  ? t("settings.models.gpu_none", "No supported GPU detected — running on CPU.")
                  : gpuStatus.ollama_accelerated
                    ? t("settings.models.gpu_ready", "Accelerated ({{vendor}})", { vendor: gpuStatus.vendor })
                    : t("settings.models.gpu_not_ready", "{{vendor}} detected — not yet accelerated", { vendor: gpuStatus.vendor })}
              </div>
            </div>
          </div>
          {gpuStatus.vendor !== "other" && !gpuStatus.ollama_accelerated && (
            <Button size="sm" variant="ghost" onClick={handleGpuRetry} disabled={gpuRetrying}>
              {gpuRetrying ? (
                <><Loader2 className="w-3.5 h-3.5 animate-spin mr-1" />{t("settings.models.gpu_retrying", "Setting up…")}</>
              ) : (
                t("settings.models.gpu_retry", "Retry GPU setup")
              )}
            </Button>
          )}
        </div>
