...
export interface GpuRuntimeStatus {
  ...
  vendor: string;
  ollama_accelerated: boolean;
  image_accelerated: boolean;
}

// Windows-only GPU-runtime provisioning status (Ollama's own GPU runner
// and the local image backend's sd.cpp build are separate subsystems —
// see gpu_runtime_status in commands/hardware.rs). Returns
// platform_supported: false on macOS.
export function gpuRuntimeStatus(): Promise<GpuRuntimeStatus> {
  return tauriInvoke("gpu_runtime_status");
}

export function gpuRuntimeRetryOllama(): Promise<void> {
  return tauriInvoke("gpu_runtime_retry_ollama");
}

export function gpuRuntimeRetryImage(): Promise<void> {
  return tauriInvoke("gpu_runtime_retry_image");
}
