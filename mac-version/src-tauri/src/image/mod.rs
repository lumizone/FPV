...
/// Reject image URLs that could target local/private services, including DNS
/// names resolving to private addresses and IPv4-mapped IPv6 literals.
pub(crate) async fn validate_public_image_url(url: &url::Url) -> AppResult<()> {
    use std::net::IpAddr;
    ...
    let addresses = if let Ok(ip) = host.parse::<IpAddr>() {
        ...
        tokio::net::lookup_host((host, url.port_or_known_default().unwrap_or(443)))
            .await
            .map_err(|_| AppError::Other("image URL host could not be resolved".into()))?
...
#[cfg(test)]
mod tests {
    use super::{validate_png_payload, validate_public_image_url, ImageProvider};
    ...
    #[tokio::test]
    async fn image_urls_reject_private_and_mapped_addresses() {
        for value in [
            ...
            assert!(validate_public_image_url(&url).await.is_err(), "{value}");
        }
    }
