...
fn ollama_env_vars(
    ...
    vec![
        ("OLLAMA_HOST", std::ffi::OsString::from("127.0.0.1:11434")),
        (
            "OLLAMA_LIBRARY_PATH",
...
async fn wait_for_ready(client: &OllamaClient, timeout: Duration) -> AppResult<()> {
    ...
    Err(AppError::Sidecar(format!(
        "ollama sidecar did not become ready within {timeout:?}"
    )))
}
...
mod windows_tests {
    ...
    fn ollama_host_is_pinned_to_loopback_not_inherited() {
        ...
        assert_eq!(
            find(&vars, "OLLAMA_HOST").as_deref(),
    ...
    fn max_loaded_models_matches_semantic_memory_tiering() {
        ...
        assert_eq!(
            find(&vars, "OLLAMA_MAX_LOADED_MODELS").as_deref(),
            Some("2")
        );
